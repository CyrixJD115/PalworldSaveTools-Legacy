...
Palworld Save Tools（PST）是一款快速的一体化桌面应用程序，用于检查和编辑《幻兽帕鲁》存档文件。它使用 Python 和 PySide6 构建，可以直接读写游戏的压缩存档格式，无需安装游戏模组。

无论是管理专用服务器、在联机与专用服务器存档之间迁移、清理废弃数据，还是精细调整单个帕鲁，PST 都能通过统一界面完成。

### 亮点

- **跨平台** — 适用于 **Windows**、**Linux** 和 **macOS** 的预构建二进制文件。
- **快速本机解析** — 最快的存档文件读取器之一，由 [`palsav`](../../src/palsav) 引擎提供支持。
- **视觉地图** — 带有基地/玩家标记、禁区和坐标校准的交互式世界地图。
- **深度帕鲁编辑** — 完全控制统计数据、IV、灵魂、技能、被动技能、工作适应性、等级和外观标志。
- **服务器级工具** — 为管理员构建的批量删除、清理、转换和角色转移。
- **自动备份** — 每个保存操作都会在写入之前创建备份。
- **10 种语言** — 提供本地化界面、应用内指南和文档，并可从本页顶部快速切换。
...
|类别 |你能做什么|
|---|---|
| **玩家管理** |编辑名称、等级、统计数据、技术点。批量管理物品、帕鲁、跨玩家的技术。清理不活跃或重复的玩家。 |
| **帕鲁编辑器** |改变统计数据，IV，灵魂，等级，技能，被动技能，工作适应性，头目/幸运标志。导出/导入pals。检测并修复非法pals。无上限编辑的作弊模式。 |
| **公会管理** |重命名公会、更换领导者、设定等级。解锁实验室研究。在公会之间移动玩家。删除空的或不活跃的公会。 |
| **据点工具** |查看所有基地的公会信息。导出/导入蓝图。将基地克隆到其他公会。在地图上重新定位基地。调整半径。删除不活跃的据点。 |
| **地图查看器** |带有基地和玩家标记的交互式世界地图。绘制禁区。校准模式。世界地图和树状地图视图。缩放、平移、飞至。 |
| **库存管理** |编辑玩家物品、关键物品、装备栏位。解锁所有快速旅行点。浏览和编辑所有公会的据点物品栏和容器。管理据点工作帕鲁。 |
| **排除** |通过持久的排除列表保护玩家、公会和基地免遭清理。从上下文菜单添加条目。 |
| **保存工具** |在 SAV 和 JSON 之间转换保存。将 GamePass 转换为 Steam。在世界之间转移角色。修复主机存档。恢复地图进度。扩展帕鲁终端栏位。 |
| **清洁和公用事业** |删除空公会、不活跃的基地/玩家、未引用的数据。删除无效项目/帕鲁/结构。重置地下城、石油钻井平台、补给品。修复时间戳。 |

### 玩家管理

- 按姓名、等级、帕鲁计数、UID、公会和最后上线时间查看和搜索所有玩家。
- 编辑玩家姓名、等级、统计数据和技术点。
- **统计选项卡** — 英雄统计数据（生命值、耐力、攻击、防御、工作速度、体重）以及正确的游戏内计算值；具有切换器和旋转器的遗物能力。
- **最大所有统计数据** — 立即将所有统计数据限制为最大（50 点）。
- **跨多个玩家的批量操作**：物品管理、帕鲁管理和技术解锁。
- 按时间阈值删除不活跃玩家；删除重复项。

### 帕鲁编辑器

适用于任何玩家拥有的任何帕鲁的深度编辑界面。 帕鲁由 **队伍**（现役小队）和 **帕鲁终端**（存储）组织。

- **统计数据和 IV** — 生命值、攻击力、防御力 (IV 0–100)、等级 (1–80)、信任等级 (0–10)。
- **灵魂** — 生命值、攻击力、防御力、工作速度 (0–20)。
- **技能** — 主动技能选择器；学习所有动作；跨帕鲁的批量同步技能。
- **被动技能** - 具有完整游戏数据的被动选择器。
- **工作适应性** — 设置个人工作适应性等级（0–10）。
- **外观标志** — 切换 头目/Alpha、幸运/闪光、暴走、觉醒 和 导入/DNA。
- **排名和锁定** — 设置排名和最喜欢的锁定级别 (0–3)。
- **作弊模式** — 切换以展开所有大写：等级、IV、灵魂、冷凝等级至 255；解锁无限的主动/被动技能，并允许重复。
- **导出/导入** — 右键单击​​任何帕鲁以导出为 `.pstpal`（压缩）或 `.json`。导入到队伍、帕鲁终端、DPS 或据点工作帕鲁的空栏位中。适用于保存和玩家。
- **最大化所有帕鲁** — 最大化队伍中所有帕鲁、所有帕鲁终端页面或所有据点工作帕鲁的所有统计数据（IV、灵魂、等级、等级）— 尊重作弊模式上限。
- **修复非法帕鲁** — 检测并限制帕鲁的每个玩家的非法统计数据、技能或特征。
- **批量克隆/删除** — 物种选择器对话框，带有用于批量操作的数量控制和源切换（队伍/帕鲁终端/DPS）。
- 添加新的帕鲁或双击快速删除。
...
- **据点微移** — 按精确的 X/Y/Z 偏移微移据点以修复地面剪切或浮动。
- **调整基础半径** (50%–1000%)。
- 删除不活动的基地和非基地地图对象。
...
- 武器、配件、食物、盔甲、盾牌、滑翔机和模块栏位的设备面板。
- 一键解锁所有地图+快速旅行点。

**基地库存** — 浏览和管理所有基地的物品和工作帕鲁：
- 查看/编辑容器中的项目；透明容器；修改容器栏位。
- 跨公会物品操作（查找/删除所有公会的物品）。
- 跨公会结构删除。
- **据点帕鲁** 子选项卡 — 使用完整的帕鲁编辑器上下文菜单管理分配给每个据点的工作帕鲁。
...
- **清理** — 删除无效/修改的项目、无效的帕鲁和 被动技能、无效的结构；修复非法帕鲁 （上限为合法最大值）；重置防空炮塔；解锁private chests；修复所有结构。
...
| **Linux** | `PalworldSaveTools-*-linux.AppImage` |任何现代发行版 |
...
> **Linux:** 您可能需要将文件标记为可执行文件：`chmod +x PalworldSaveTools-*-linux.AppImage`
...
1. **加载您的保存**
   - 单击 **菜单 → 加载存档**，或将 `.sav` 文件拖放到窗口上。
   - 导航到您的 Palworld 存档文件夹并选择 `Level.sav`。

2. **探索您的数据**
   - 使用选项卡 - **地图**、**工具**、**玩家**、**公会**、**基地**、**玩家物品栏**、**基地库存**、**帕鲁编辑器**、**排除** - 探索您的保存。
...
**主机/合作社 (Windows)：**
```
%localappdata%\帕鲁\Saved\SaveGames\YOURID\RANDOMID\
```

**专用服务器：**
```
steamapps\common\Palworld\帕鲁\Saved\SaveGames\0\RANDOMSERVERID\
```
...
**工作原理：** 合作模式存档使用 `0001.sav` 作为主机玩家。专用服务器则不然——每个玩家都有一个常规的 UID。修复主机存档**交换**两个玩家文件（如交易席位），而不是副本。您在 `0001.sav` 中的合作角色会被交换到服务器的栏位中。

1. **将你的合作存档复制到服务器。**
   - 合作保存位置：`%localappdata%\帕鲁\Saved\SaveGames\YOURID\RANDOMID\`
   - 从那里复制 `Level.sav` 和 `Players` 文件夹。
   - 粘贴到服务器存档文件夹：`steamapps\common\Palworld\帕鲁\Saved\SaveGames\0\RANDOMSERVERID\`
...
4. **启动服务器。**
   - 你原来的合作角色（所有进度，帕鲁，基础）现在已链接到服务器。临时占位符消失了。
...
**工作原理：** 相同的 GUID 反向交换 — 修复主机存档 **交换**两个文件，而不是副本。你的服务器角色（常规 UID）会被交换到 `0001.sav` （主机槽），这样你就可以在服务器进程中主持合作游戏。

1. **将服务器保存的内容复制到本地PC。**
   - 服务器保存位置：`steamapps\common\Palworld\帕鲁\Saved\SaveGames\0\RANDOMSERVERID\`
   - 从那里复制 `Level.sav` 和 `Players` 文件夹。
   - 粘贴到您的本地合作文件夹：`%localappdata%\帕鲁\Saved\SaveGames\YOURID\RANDOMID\`
...
4. **正常主持合作社。**
   - 您的服务器角色现在是主机（`0001.sav`）。所有进展，帕鲁，并且基础完好无损。
...
**关键概念 - 修复主机存档总是交换两个玩家的问题。** 它交换他们的存档文件，就像两个人交换座位一样。它不会将一个复制到另一个上。在任何交换之后，两个玩家仍然存在——他们只是在不同的文件中。

由于一次交换将玩家 B 移动到主机槽中，但将玩家 A 的数据保留在 B 的旧文件中，因此需要第二次交换才能将玩家 A 的原始角色放回原处。方法如下：
...
> **为什么要进行两次交换？** 修复主机存档 **交换**两个文件 - 它不是副本。第一次交换将 B 放入主机栏位中，但 A 的数据最终出现在 B 的旧 UID 中（游戏中不再存在）。第二次交换将 A 的数据移至 A 的新客户端 UID 中。两次交换，所有进度均保留。
...
将一名玩家（包含所有帕鲁、库存、技术和进度）从一个世界复制到另一个世界 - 对于在合作世界和专用服务器或两个服务器之间移动角色非常有用。

**工作原理：**与“修复主机存档”（**交换**两个文件）不同，角色传输**将**玩家从一个 `Level.sav` 复制到另一个 `Level.sav` 中。源保存未受影响。
...
6. 单击**传输**。角色、帕鲁、库存和公会成员资格将复制到目标保存。
7. 保存更改。创建自动备份。
...
存档文件格式已过时。在游戏中加载存档（单人、合作或专用服务器）一次以触发自动结构更新，然后重试。确保在最新游戏补丁时或之后更新了保存。
...
- **Linux:** `chmod +x PalworldSaveTools-*-linux.AppImage` 将其标记为可执行。
...
**使用此工具的风险由您自行承担。在进行任何修改之前，请务必备份您的存档文件。**

开发人员对任何存档数据丢失或使用此工具可能出现的问题不承担任何责任。
...
- **Palworld 模组社区** — 模组制作者、工具开发者和修补者，他们分享知识、逆向工程格式并推动生态系统向前发展。这个项目是建立在集体努力的基础上的。
